...
bool Sprite::LoadTextureFromFile(const char *filename) {
  ...
  unsigned char *image_data =
      stbi_load(newFilename.c_str(), &image_width, &image_height, NULL, 4);
  if (image_data == NULL) {
    _size = ImVec2(0, 0);
...
ImTextureID Sprite::_loadTextureFromMemory(const unsigned char *image_data,
...
ImTextureID Sprite::_loadTextureFromMemory(const unsigned char *image_data,
  ...
  extern ID3D11Device
      *g_pd3dDevice; // Add this line if g_pd3dDevice is defined elsewhere
  ...
  hr = g_pd3dDevice->CreateShaderResourceView(pTexture, &srvDesc,
                                              &shaderResourceView);
  pTexture->Release();
